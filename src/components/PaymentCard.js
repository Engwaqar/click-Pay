...
const PaymentCard = ({source,active,title,onPress,Number}) => {
  return (
    <TouchableOpacity onPress={onPress} style={[styles.container,{backgroundColor:active?colors.primary:colors.lighterGrey}]}>
      <Icon size={50} source={source} />
      <View style={{ padding: 5,flex:1,marginLeft:8 }}>
        <ResponsiveText color={active?colors.white:colors.black} >{title}</ResponsiveText>
      ...
      <View style={{
           ...
           >
                {active ? <Icon source={globalPath.RADIO_DOT} size={10} tintColor={colors.white}/> :<View/>}
            </View>
    </TouchableOpacity>
  );
};
